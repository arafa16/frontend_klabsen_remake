...
const createEmployeePage = () => {
  ...
  const [is_active, set_is_active] = useState('');
    ...
    const {submit:submitCreate, message:messageUserCreate, isLoading: isLoadingUserCreate} = createDataUser({
        nik:nik,
        absen_id:absen_id,
        name:name, 
        password:password,
            ...
            <form onSubmit={submitCreate}>
                <div className={`flex items-center justify-center gap-x-4 col-span-12 mt-10 intro-y sm:justify-end`}>
                    ...
                    <Button
                        ...
                        type='submit'
                        >
                        {isLoadingUserCreate ? <LoadingIcon icon="circles" className="w-4 h4" color="white"  /> : 'Save'}
                    </Button>
                </div>
