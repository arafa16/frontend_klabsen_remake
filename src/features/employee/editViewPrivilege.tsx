...
const editViewPrivilege = (props: any) => {
  ...
  useEffect(() => {
    if (datas && datas.privilege_id !== null) {
      setPrivilege({
        uuid: datas?.privilege?.uuid,
        dashboard: datas?.privilege?.dashboard ? 1 : 0,
        edit_user_sub: datas?.privilege?.edit_user_sub ? 1 : 0,
        absen: datas?.privilege?.absen ? 1 : 0,
        kalendar_sub: datas?.privilege?.kalendar_sub ? 1 : 0,
        absen_modal: datas?.privilege?.absen_modal ? 1 : 0,
        wfh_modal: datas?.privilege?.wfh_modal ? 1 : 0,
        shift_modal: datas?.privilege?.shift_modal ? 1 : 0,
        on_site_modal: datas?.privilege?.on_site_modal ? 1 : 0,
        absen_check: datas?.privilege?.absen_check ? 1 : 0,
        admin_event: datas?.privilege?.admin_event ? 1 : 0,
        perhitungan_absen: datas?.privilege?.perhitungan_absen ? 1 : 0,
        pengajuan_koreksi_sub: datas?.privilege?.pengajuan_koreksi_sub ? 1 : 0,
        approval_koreksi_sub: datas?.privilege?.approval_koreksi_sub ? 1 : 0,
        approval_all_koreksi_sub: datas?.privilege?.approval_all_koreksi_sub
          ? 1
          : 0,
        employees: datas?.privilege?.employees ? 1 : 0,
        data_employee: datas?.privilege?.data_employee ? 1 : 0,
        slip_gaji: datas?.privilege?.slip_gaji ? 1 : 0,
        pendapatan_sub: datas?.privilege?.pendapatan_sub ? 1 : 0,
        pendapatan_lain_sub: datas?.privilege?.pendapatan_lain_sub ? 1 : 0,
        pendapatan_admin_sub: datas?.privilege?.pendapatan_admin_sub ? 1 : 0,
        attribute: datas?.privilege?.attribute ? 1 : 0,
        setting: datas?.privilege?.setting ? 1 : 0,
        overtime: datas?.privilege?.overtime ? 1 : 0,
        overtime_user: datas?.privilege?.overtime_user ? 1 : 0,
        overtime_superior: datas?.privilege?.overtime_superior ? 1 : 0,
        overtime_assignor: datas?.privilege?.overtime_assignor ? 1 : 0,
        overtime_admin: datas?.privilege?.overtime_admin ? 1 : 0,
        id_privilege: datas?.privilege?.id,
      });
    }
  }, [datas]);
