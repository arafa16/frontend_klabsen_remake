  ...
  useEffect(() => {
    if (data && isSuccess) {
      if (!isLoading) {
        ...
        countData(
          data && data.datas && data.datas.data && data.datas.data.count,
        );
...
export const getDataPendapatansTable = () => {
  ...
  const [allPage, setAllPage] = useState(0);
  let [dataOption, setDataOption] = useState<any>({
    pendatapan_atas: [],
    periode: [],
    group: [],
  });
  ...
  useEffect(() => {
    if (data && isSuccess) {
      if (!isLoading) {
        data.datas.meta.pendapatan_atas_select.map((item: any) => {
          setDataOption((prev: any) => ({
            ...prev,
            pendatapan_atas: [
              ...prev.pendatapan_atas,
              { name: item.pendapatan_atas },
            ],
          }));
        });
        data.datas.meta.year_select.map((item: any) => {
          setDataOption((prev: any) => ({
            ...prev,
            periode: [...prev.periode, { name: item.name }],
          }));
        });
        data.datas.meta.group_select.map((item: any) => {
          setDataOption((prev: any) => ({
            ...prev,
            group: [...prev.group, { name: item.name, uuid: item.uuid }],
          }));
        });
        setDataResult(data && data.datas && data.datas.data);
        countData(
  ...
  useEffect(() => {
    const delayDebounceFn = setTimeout(() => {
      ...
      const paramsObj: any = {
        page,
        limit,
        pendapatan_atas: search.pendatapan_atas,
        group_uuid: search.group_uuid,
...
export const getDataPendapatansById = (datas: any) => {
  ...
  const {
    data: pendapatan,
    isSuccess: isPendapatanSuccess,
    isLoading: isPendapatanLoading,
  } = useSelector((state: any) => state.pendapatan);
  ...
  useEffect(() => {
    const totalPengeluaran =
      Number(data.zakat) +
      Number(data.iuran_koperasi) +
      Number(data.potongan_pinjaman) +
      Number(data.potongan_jht) +
      Number(data.potongan_bpjs) +
      Number(data.potongan_pensiun) +
      Number(data.adjustment_minus) +
      Number(data.pph21) +
      Number(data.tunjangan_jht) +
      Number(data.tunjangan_pensiun) +
      Number(data.tunjangan_jkk) +
      Number(data.tunjangan_jkm) +
    ...
    const totalPendapatan =
      ...
      Number(data.incentive) +
      Number(data.kjk) +
      Number(data.rapel) +
      Number(data.adjustment) +
      Number(data.overtime_allowance) +
      Number(data.tunjangan_jht) +
      Number(data.tunjangan_pensiun) +
      Number(data.tunjangan_jkk) +
      Number(data.tunjangan_jkm) +
    ...
    generateQR(
      import.meta.env.VITE_REACT_APP_URL + "/pendapatan/slip/" + data.uuid,
    );
  }, [data]);
