...
export const SlideOverEditDate = (props: any) => {
  ...
  const {
    data: inOuts,
    message: messageInOuts,
    isSuccess: isInOutsSuccess,
    isLoading: isInOutsLoading,
  ...
  useEffect(() => {
    if (pelanggaran && isPelanggaranSuccess) {
      if (!isPelanggaranLoading) {
        setDataPelanggaran(
          pelanggaran && pelanggaran.datas && pelanggaran.datas.data,
        );
        resetPelanggaran();
      }
    }
  ...
  useEffect(() => {
    if (isStatusInoutSuccess && statusInout) {
      if (!isStatusInoutLoading) {
        setDataStatusInOut(
          statusInout && statusInout.datas && statusInout.datas.data,
        );
  ...
  useEffect(() => {
    if (isJamOperasionalSuccess && jamOperasional) {
      if (!isJamOperasionalLoading) {
        setDataJamOperasional(
          jamOperasional && jamOperasional.datas && jamOperasional.datas.data,
        );
              ...
              <div className="mt-3">
                ...
                >
                  ...
                  {dataTipeAbsen.map((data: any, key) => (
                    <option key={key} value={data && data.uuid}>
                      {data && data.name}
                    </option>
                  ))}
                </FormSelect>
              ...
              <div className="mt-3">
                ...
                >
                  ...
                  {dataJamOperasional.map((data: any, key) => (
                    <option key={key} value={data && data.uuid}>
                      {data.jam_masuk}-{data.jam_pulang} {data.name}
                    </option>
                  ))}
                </FormSelect>
              </div>
