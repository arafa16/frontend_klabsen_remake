...
export const SlideOverDateKoreksiUser = (datas: any) => {
  ...
  const {
    ...
    isError,
    isLoading,
  } = useSelector((state: any) => state.koreksi);
  ...
  useEffect(() => {
    ...
    } else if (isError && messageKoreksi) {
      if (!isLoading) {
        setMessage(messageKoreksi?.data);
        dispatch(resetKoreksis());
        dispatch(getInOutsByUser({ uuid }));
        resetValue();
        setOpen(false);
  ...
  }, [messageKoreksi, isSuccess, isLoading, isError]);
                ...
                >
                  ...
                  {dataTipeAbsen.map((data: any, key) => (
                    <option
                      ...
                      className={`${data && data.is_select ? "" : "hidden"}`}
                    >
                      {data && data.name}
                    </option>
                  ))}
