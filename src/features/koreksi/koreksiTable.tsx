...
export const viewKoreksiByDate = () => {
  ...
  const [link_back, set_link_back] = useState("");
          ...
          {datas &&
            datas.map((data: any, index: any) => (
              <div
                ...
                onClick={() =>
                  navigate(
                    `/koreksi/view?${new URLSearchParams({ uuid: data.uuid, code: `${status_code}`, link_back: `${link_back}` }).toString()}`,
                  )
                }
              >
                <div
  ...
  return { showData, setDatas, set_link_back, set_status_code };
};
